...
def validate_cpf(stringvar: tk.StringVar, button: tk.Button):
    ...
    if validador_cpf(cpf):
        button.config(text="OK", activeforeground="green", foreground="green")
    else:
        button.config(text="Inválido", activeforeground="red", foreground="red")
...
def validador_gerador_gui(root, validate_cpf, generate_cpf):
    main_title = tk.Label(
        root, text="Gerador/Validador de CPF", bg="#fff", font=("Helvetica", 12, "bold")
    )
    main_title.grid(row=0, column=0, columnspan=3, pady=(0, 20))

    validate_label = tk.Label(
        root,
        text="Validar:",
        bg="#fff",
    )
    ...
    validate_entry = tk.Entry(
        root, bd=5, relief="flat", textvariable=validate_stringvar, highlightthickness=2
    )
    ...
    generate_entry = tk.Entry(
        root, bd=5, relief="flat", textvariable=generate_stringvar, highlightthickness=2
    )
    ...
    generate_button.configure(
        command=lambda: generate_cpf(generate_stringvar, validate_button)
    )
